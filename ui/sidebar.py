...
def render_sidebar() -> tuple[str, SEOProvider, dict]:
    ...
    with st.sidebar:
        ...
        has_key = bool(settings.seo_api_key)
        provider_options = ["SEMrush API", "Scraper (no API key)"]
        default_idx = 0 if has_key else 1

        provider_choice = st.radio(
            "Provider",
            provider_options,
            index=default_idx,
            help="SEMrush API requires a valid SEO_API_KEY in .env",
        )

        if provider_choice == "SEMrush API" and not has_key:
            st.warning("No SEO_API_KEY found in .env — falling back to Scraper.")
